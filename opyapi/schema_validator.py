...
from opyapi.validators.object_validators import (
...
from opyapi.validators.type_validators import (
    validate_array,
    validate_boolean,
    validate_enum,
    validate_integer,
...
def build_validator_for(schema: Dict[str, Any]) -> Callable:
    if "anyOf" in schema:
    ...
    if "not" in schema:
        return partial(validate_not, validator=build_validator_for(schema["not"]))
...
def _build_all_of_validator(items: List) -> Callable:
    ...
    validators = [build_validator_for(item) for item in schemas]
    ...
    if "items" in definition:
        if isinstance(definition["items"], list):
            ...
            validators.append(
                partial(
                    validate_items,
                    item_validator=build_validator_for(definition["items"]),
                )
            )
...
def _build_tuple_validator(definition: Dict[str, Any], validators: List[Callable]) -> None:
    items_validators = [build_validator_for(item_schema) for item_schema in definition["items"]]
        ...
        if isinstance(definition["additionalItems"], bool):
            ...
            validators.append(
                partial(
                    validate_tuple,
                    item_validator=items_validators,
                    additional_items=build_validator_for(definition["additionalItems"]),
                )
            )
    else:
...
def _build_object_validator(definition: Dict[str, Any]) -> Callable:
    ...
    if "propertyNames" in definition:
        ...
        validators.append(
            partial(
                validate_object_property_names,
                property_names=build_validator_for(definition["propertyNames"]),
            )
        )
    ...
    if "additionalProperties" in definition:
        if isinstance(definition["additionalProperties"], bool):
            ...
            property_validator_settings["additional_properties"] = build_validator_for(
