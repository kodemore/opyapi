...
def test_can_build_validator_for_boolean_type() -> None:
    validate = build_validator_for({"type": "boolean"})
...
def test_can_build_validator_for_number_type() -> None:
    ...
    # validate number
    validate = build_validator_for({"type": "number"})
    assert validate(1.0)
    with pytest.raises(TypeValidationError) as e:
        validate("4")
    assert e.value.args[0] == (
    ...
    # validate enum
    validate = build_validator_for({"type": "integer", "enum": [1, 3, 6]})
    assert validate(1)
    assert validate(3)
    assert validate(6)
...
def test_can_build_validator_for_string_type() -> None:
    ...
    # validate pattern
    validate = build_validator_for({"type": "string", "pattern": "[0-9][a-e]"})
    assert validate("1a")
    assert validate("0a")
    assert validate("9e")
    with pytest.raises(FormatValidationError):
    ...
    # validate min length
    validate = build_validator_for({"type": "string", "minLength": 3})
    assert validate("aaa")
    ...
    # validate max length
    validate = build_validator_for({"type": "string", "maxLength": 3})
    assert validate("aa")
    assert validate("aaa")
    with pytest.raises(LengthValidationError):
        validate("aaaa")
...
def test_can_build_validator_for_array() -> None:
    ...
    # validate min items
    validate = build_validator_for({"type": "array", "minItems": 2})
    assert validate([1, 2])
    with pytest.raises(LengthValidationError):
        validate([1])

    # validate max items
    validate = build_validator_for({"type": "array", "maxItems": 2})
    assert validate([1, 2])
    with pytest.raises(LengthValidationError):
        validate([1, 2, 3])
